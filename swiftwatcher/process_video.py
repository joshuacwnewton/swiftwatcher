...
class FrameStack:
    ...
    """
    def __init__(self, video_directory, filename, stack_size):
        ...
        if not self.stream.isOpened():
            ...
            self.src_framecount = self.stream.get(cv2.CAP_PROP_FRAME_COUNT)
    ...
    def save_frame(self, index, save_directory):
        ...
        if not os.path.isdir(save_directory+"/"+time[0]+":"+time[1]):
            try:
                os.mkdir(save_directory+"/"+time[0]+":"+time[1])
            except OSError:
                print("[!] Creation of the directory %s failed." % save_directory+"/"+time[0]+":"+time[1])

        try:
            cv2.imwrite("{0}/frame{2}_{1}.jpg".format(save_directory+"/"+time[0]+":"+time[1],
                                                      self.indices[index],
    ...
    def crop_frame(self, index, corners):
    ...
    def resize_frame(self, index, scale_percent):
        s = scale_percent/100
